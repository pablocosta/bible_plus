...
capitulo   = []
livro      = []
for k, v in tqdm.tqdm(utils.dictOldBookNVIPT.items()):
...
polars.concat(listDf).write_csv("./data/NVIPTOld.tsv", separator="\t")
...
capitulo   = []
livro      = []
for k, v in tqdm.tqdm(utils.dicNewBookNVIPT.items()):
...
polars.concat(listDf).write_csv("./data/NVIPTNew.tsv", separator="\t")