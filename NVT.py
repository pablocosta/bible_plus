...
capitulo   = []
livro      = []
for k, v in tqdm.tqdm(utils.dictOldBookNVT.items()):
...
polars.concat(listDf).write_csv("./data/NVTOld.tsv", separator="\t")
...
capitulo   = []
livro      = []
for k, v in tqdm.tqdm(utils.dicNewBookNVT.items()):
...
polars.concat(listDf).write_csv("./data/NVTNew.tsv", separator="\t")