...
for version in ["HOF", "LUTH1545", "NGU-DE"]:
    ...
    capitulo   = []
    livro      = []
    for k, v in tqdm.tqdm(utils.dictGermanOld.items()):
        
        for chapter in range(1, v+1):
            url = baseUrl + "search=" + k + "%20" + str(chapter) + "&version=" + version
            print(url)
            dicionarioUnidade = utils.crawlSiteGER(url)
    ...
    polars.concat(listDf).write_csv("./data/DE/"+version+"Old.tsv", separator="\t")
    ...
    capitulo   = []
    livro      = []
    for k, v in tqdm.tqdm(utils.dictGermanNew.items()):
    ...
    polars.concat(listDf).write_csv("./data/DE/"+version+"New.tsv", separator="\t")